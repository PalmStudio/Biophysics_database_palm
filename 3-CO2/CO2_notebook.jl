...
The chambers allows for finely controling the radiation in the visible spectrum with four LED lamps, and the climatic conditions, including the temperature, relative humidity and CO2 concentration.
...
Of course this assumption is just an assumption, as the response time depends on the volume of the chamber, which can delay the response. We can compute it using the chamber volume divided by the air flow (m3 h-1) though. The measurement of the fluxes are delayed by the response time of the system. For example, if we shut down the light abruptly in the chamber, the response of the plant would be close to immediate (the photosynthesis would go down to zero), but we would still measure photosynthesis for a bit of time in the analyzer because of the time response of the system, as the volume of air in the chamber is big compared to the fluxes.
...
flag_before_opening = 1

# ╔═╡ 671fe6aa-6f96-43ce-8372-841819b8ce66
flag_after_opening = 9
...
df = let 
	...
	transform!(df_, :door_event => ByRow(x-> ismissing(x) ? false : true) => :door_event)

	transform!(
		df_,
		:door_event => (x -> propagate_around(x, true, 1, 9)) => :opening_around
	)
	df_
end
