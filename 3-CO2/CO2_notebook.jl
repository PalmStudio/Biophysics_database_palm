...
# ╔═╡ 452f28ad-7854-429c-8d0a-f09f830d57b8
begin
...
# ╔═╡ 2b4f56ce-b2be-11ed-2d56-ddbd98334621
md"""
# CO2 fluxes
...
climate_5min = let
	df = CSV.read("../1-climate/climate_mic3_5min.csv", DataFrame)
...
md"""
The opening and closing of the chamber's door is monitored continuously and available in the `Mic3_ouverture_porte.csv` file. The `ouverture_porte` columns, renamed in this notebook into `door_event` has the value of 0 when a closing event of the door was registered, and 1 when it is opening.
"""
...
door_df = let
	...
	select!(
		...
		:ouverture_porte => :door_event,
		:DateTime => (x -> round.(Dates.DateTime.(x, "yyy-mm-dd HH:MM:SS"), Dates.Minute(10))) => :DateTime_10min,
	)

	combine(
		groupby(df, :DateTime_10min),
		:door_event => (x -> true) => :door_event
	)
end
...
md"""
!!! note
	Note that we also added a variable that flags all data points around a door opening.
"""
...
md"""
*Figure 1. CO2 flux in the chamber during the whole experiment (2 months). Points are colored green when the system is not at steady state because there is a change of scenario. These points will be removed from the final database.*
"""

# ╔═╡ e8742a04-63ee-445a-8f27-cd1a1e226767
md"""
*Figure 2. CO2 flux in the chamber during the whole experiment (2 months). Points are colored orange (true) when the system is not at steady state because there was a door opening / closing event near the point. These points will be removed from the final database.*
"""

# ╔═╡ 86163cb6-d200-454a-8daf-d8d5e4ea47d7
md"""
*Figure 3. CO2 concentration in the chamber measured at valve opening. All points that are in-between twwo scenarii are flagged here (see below for the code)*
"""
...
md"""
The CO2 fluxes are not reliable when there is a sudden change in the conditions due to an opening of the door of the chamber, or when there is a change in the CO2 scenario in the chamber, for example when changing from the 400ppm scenario to the 600ppm scenario.

We then have to remove all points around these events.
"""
...
Saving the resulting database to disk.
"""
...
function propagate_around(x, val, before, after)
		...
		val_in_x = findall(skipmissing(x) .== val)
...
df = let 
	...
	# Adding "opening_around" column that flags values around a door opening or closing event. Also adding "CO2_change_around" column that flags values that are close to a scenario change (1 before and 5 after a change).
	transform!(
		df_,
		:door_event => (x -> propagate_around(x, true, flag_before_opening, flag_after_opening)) => :opening_around,
			:CO2_change => (x -> propagate_around(x, true, flag_before_scenario, flag_after_scenario)) => :CO2_change_around
	)
	df_
end
...
mapping(
	...
	color=:CO2_change_around => string => "Around CO2 \nscenario change",
) |> draw

# ╔═╡ 2d872c06-048e-47a8-970e-206910476935
data(df) *
mapping(
	:DateTime => "Time",
	:flux_umol_s => "CO2 flux (μmol s⁻¹)",
...
df_filt = let
	...
	filter!(
		x -> x.opening_around == false && x.CO2_change_around == false && x.CO2_dry_MVP_change == false,
		df_
	)
...
max_system_response_time = Dates.Minute(round(unique(df_filt.flow_m3_h)[1] / 1.5 * 60))
