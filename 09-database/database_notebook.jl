...
# ╔═╡ 2072d6f3-29d6-437c-9dea-5b7ab78c3040
md"""
Scenarios were initially identified from the treatment schedule assigned to each day and plant. However, conditions inside the chamber did not change abruptly at midnight, but rather transitioned progressively from one scenario to the next. As a result, if the analysis requires conditions to match the exact specifications of a given scenario, it is preferable to rely on an alternative identification based on the actual chamber conditions. This information is available in `00-data/scenario_sequence/Tag_database.csv`, which records the effective scenario in the chamber and flags a number of visually identified outliers.
"""

# ╔═╡ f5653a7b-5ca7-4f2a-b9ac-8fca1b4cdfa4
scenario_id = CSV.read("../00-data/scenario_sequence/scenario_id_actual.csv", DataFrame, dateformat="yyyy-mm-ddTHH:MM:SSZ")
...
db_5min = let
    ...
    for (i, row) in enumerate(eachrow(df_sequence_params))
        ...
        # Don't compute time-steps that cover two sequences (we are changing the plant here):
        if length(timestamps_within) > 0
...
db_10min = let
    db_ = leftjoin(CO2, climate_10min, on=[:DateTime_start_input => :DateTime_start], makeunique=true)
    db_ = leftjoin(db_, transpiration_10min, on=[:DateTime_start_input => :DateTime_start], makeunique=true)
