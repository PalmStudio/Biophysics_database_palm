...
gr_CO2=CurveCO2 %>% 
  ...
  geom_text(data=param_sub,aes(x=Cᵢ, y=A, label=paste('Vcmax:',round(VcMaxRef,2),'\n','Jmax:',round(JMaxRef,2),'\n','TPU:',round(TPURef,2),'\n','Rd:',round(RdRef,2))),
            hjust=0,col=2)+
  myTheme_multi+
  labs(x=expression(C[i]*' (ppm)'),
...
gr_gs=CurveGs %>% 
  ...
  myTheme_multi+
  geom_text(data=param_sub,aes(x=Dₗ, y=Gₛ, label=paste('g0:',round(g0,2),'\n','g1:',round(g1,2))),
            hjust=0,col='blue')+
  labs(x=expression(D[l]*' (kPa)'),
...
area=data.table::fread(input = '0-data/ComparisonArea.csv') %>% 
  mutate(plant=ifelse(plant=='P5','P4',plant))
...
pLeaf=area%>%
  ...
  # facet_wrap(~plant)+
  labs(x=expression('measured leaf area '(cm**2)),
...
pPlant=area%>%
  group_by(plant)%>%
  summarize(`3D mesh`=sum(Manual_mesh_area),
  ...
  labs(y=expression('plant leaf area '(m**2)),
       x='')
...
cowplot::plot_grid(parVid,
                   plot_grid(
                   ggdraw() +
                     draw_image(picSuncscan,x = 0.1, y =-0.1, width = 1, height = 1.2),parPalm,ncol=2),ncol=1,
