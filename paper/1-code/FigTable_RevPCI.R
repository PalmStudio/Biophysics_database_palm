...
ggplot() +
  ...
  geom_text(data = cal %>% filter(event %in% c(paste0("S", 1:8),'WalzOpen','WalzClosed')), aes(x = Date, y = plant,label=scenar),angle = 90,size=3) +
  scale_x_date() +
  scale_fill_manual(values = colors_event, name = "Scenario",na.value = 'white') +
...
mic3 <- merge(mic3_raw, cal %>% filter(!is.na(scenar)) %>% filter(scenar!='Storage'), all.y = T) %>%
  rename(
...
### tests light
merge(mic3_raw %>% data.frame(), cal %>% dplyr::filter(!is.na(scenar)) %>% dplyr::select(Date,scenar) %>% distinct(), all.y = T) %>%
...
graphLeaf <- ggplot() +
  ...
  geom_line(data=th,aes(x = hms(hms), y = Tl_mean, col = sprintf(paste("Leaf", sprintf("%02d", Leaf))), group = Leaf)) +
...
fullarea %>% 
  ggplot()+
  geom_point(aes(x=Date,y=PLA,shape=scenar,col=plant,group=plant),size=3)+
  geom_line(aes(x=Date,y=PLA,shape=scenar,col=plant,group=plant))+
  labs(
...
donAll=merge(database_raw,AllScenar%>%
            filter(Plant!=''),all.y=T)

AllFCo2=donAll%>% 
  ggplot(aes(x=hms(hms),y=CO2_outflux_umol_s,col=rep,group=paste(Plant,Scenario,rep)))+
  geom_line()+
...
don=merge(database_raw,AllScenar%>%
            filter(Plant!='' & Ref=='T'),all.y=T)
