"""
    compute_jpg_temperature_batch(n, mask_dir, img_dir, csv_dir, climate; delay::Dates.TimePeriod=Dates.Second(3512))

Apply `compute_all_images` over a range of images given by `n` (easier for distributed processing).

The images are in `img_dir` and the masks are in `mask_dir`. The results are saved in `csv_dir`.
...
"""
function compute_jpg_temperature_batch(n, mask_dir, img_dir, csv_dir, climate; delay::Dates.TimePeriod=Dates.Second(3512))
    # n = partitions[7]
    ...
    df_all = compute_all_images(image_files, mask_files, climate; delay=delay)
    CSV.write(joinpath(csv_dir, "leaf_temperature_$(n[1])-$(n[end]).csv"), df_all)
end

"""
    compute_all_images(image_files, mask_files, climate; delay::Dates.TimePeriod=Dates.Second(3512))
...
with the air temperature and relative humidity. It should have the following columns:
...
"""
function compute_all_images(image_files, mask_files, climate; delay::Dates.TimePeriod=Dates.Second(3512))
    ...
    image_dates = DateTime.(basename.(image_files), dateformat"yyyymmdd_HHMMSS\\_\\R\\.\\j\\p\\g")
