# Aim: make simulation with R. Vezy's PlantBiophysics julia package with data from écotron experimenation.
# Authors: S. Treillou & V. Torrelli
# Date: 13/07/2021
#]add PlantBiophysics https://github.com/VEZY/PlantBiophysics.jl


cd("C:\\Users\\U051-J209\\Cirad\\PalmStudio - Manip EcoTron 2021\\Ecotron2021\\")
...
# Load data 
df = DataFrame(CSV.File("0-data/8-not_FSPM_modeling/climate_walz_area_5min.csv"))

uncert = false

# Simulate data
(!uncert) && (df[!, :Tleaf_sim] .= df[!,:E_sim] .= df[!, :λE_sim] .= df[!,:Rn_sim] .= df[!, :H_sim] .= df[!, :A_sim] .= df[!, :GH2O_sim] .= df[!, :ci_sim] .= df[!, :VPD_sim] .= df[!, :CO2_flux_sim] .= df[!, :transpi_sim] .= 0.)
(uncert) && (df[!, :Tleaf_sim] .= df[!,:E_sim] .= df[!, :λE_sim] .= df[!,:Rn_sim] .= df[!, :H_sim] .= df[!, :A_sim] .= df[!, :GH2O_sim] .= df[!, :ci_sim] .= df[!, :VPD_sim] .= df[!, :CO2_flux_sim] .= df[!, :transpi_sim] .= 0. ± 0.)
for i in 1:size(df,1)
    ...
    df.E_sim[i] = leaf.status.λE /(meteo.λ * constants.Mₕ₂ₒ) * 1000   # Transpiration in mol/m2/s
    ...
    df.transpi_sim[i]= df.E_sim[i] * (constants.Mₕ₂ₒ) * (df.cum_area[i]/100^2) * 600 # transpi in mL/min
    df.CO2_flux_sim[i] = leaf.status.A * (df.cum_area[i]/100^2) # CO2 flux in μmol/s
end

CSV.write("0-data/8-not_FSPM_modeling/flux_PlantBiophysics_output_5min2.csv",df[!,[:DateTime_start,:CO2_flux_sim, :cum_area, :A_sim, :transpi_sim]])
...
# function filter_df(df::DataFrame,plant::String, leaf::String, paramdate::String, date::DateTime_start)
#     if (length(plant) != 2)|(length(leaf) != 2)|(length(paramdate) != 4)
#         throw(ArgumentError("Invalid plant, leaf or parameter date names (need shape as PX, FX, XXXX)"))
#     end
...
# #after this is for plots :

# df_day = filter(x->x.Date==DateTime_start(2021,3,2),df_leaf)
