...
The image database was compressed using `tar` with the `bzip2` algorithm, which reduced significantly disk space from ~60Go to ~23Go.
...
md"""
*Figure 2. Leaf temperature for the four plants along the whole duration of the experiment. Points are colored by plant in the microcosm (`mic3`). The red colored points represent the air temperature.*
"""

# ╔═╡ 4aa263c2-75eb-4173-8f36-5dacefdaf265
p_temp = data(dropmissing(leaf_temperature_df, :Ta_measurement)) *
mapping(
...
# ╔═╡ b1c460a7-8ac3-4e62-9917-ed1febcfcac3
#save("p_temp.png", p_temp)

# ╔═╡ ca515496-d35b-42ee-9855-da2fd5fcb167
md"""
## Validating against other measurements
"""

# ╔═╡ fa43d4bc-52df-40b3-872a-3dbc0a31614f
md"""
### Radiation
"""

# ╔═╡ 92b863e8-fc17-4a57-ac4a-376927436d6e
p_rad_tair = data(dropmissing(leaf_temperature_df, :Ta_measurement)) *
mapping(
    :DateTime => (x -> Time(x)) => "Hour (UTC)",
    :R_measurement => (x -> x ./ 10) => "Radiation (μmol)",
	layout=:DateTime => Date => "Day",
) *
visual(Scatter, color=:red, markersize=1) +
AlgebraOfGraphics.data(dropmissing(leaf_temperature_df, :Ta_measurement)) *
mapping(
    :DateTime => Time => "Hour (UTC)",
    :Ta_measurement => "Leaf temperature (°C)",
    color=:plant => string => "Plant",
	layout=:DateTime => Date => "Day",
) *
visual(Scatter, markersize=3) |>
...
# ╔═╡ 92700f55-3307-486d-8a2b-9f23915be916
md"""
Note that the radiation is in red, and the leaf temperature is colored by plant. The radiation values are divided by 10 to get a shared y-scale.
"""

# ╔═╡ 4465ba7d-b4ab-4483-b565-1a36e4ca7bb7
md"""
### CO2 fluxes
"""

# ╔═╡ 68d45a83-58e2-4f77-ba78-128c458f4fac
p_rad = data(dropmissing(leaf_temperature_df, :Ta_measurement)) *
mapping(
    :DateTime => (x -> Time(x)) => "Hour (UTC)",
    :R_measurement => (x -> x ./ 10) => "Radiation (μmol)",
	layout=:DateTime => Date => "Day",
...
# ╔═╡ 2db1c591-d54a-43f4-a5b4-acca5e5495b7
CO2_flux =
    let
...
# ╔═╡ f8b76af7-47b5-413d-8ac2-ce0ae836ee4b
df_tleaf_CO2 = let
	df_CO2_minute = transform(
		CO2_flux,
		:DateTime => (x -> round.(x, Dates.Minute(1))) => :DateTime,
	)
	df_leaf_temp_minute =  transform(
		leaf_temperature_df,
		:DateTime => (x -> round.(x, Dates.Minute(1))) => :DateTime,
	)

	leftjoin(df_CO2_minute, df_leaf_temp_minute, on=:DateTime)
end;

# ╔═╡ 9e7e108e-b2d6-4bec-9501-8ff4da0512c0
p_CO2_2 = let
	df_ = dropmissing(df_tleaf_CO2, :Tl_mean)
	DataFrames.subset!(df_, :flux_umol_s => (x -> -5 .< x .< 20))
	data(df_) *
	mapping(
	    :DateTime => (x -> Time(x) + Second(0)) => "Hour (UTC)",
	    :flux_umol_s => (x -> x * 10) => "CO2_flux (μmol)",
		layout=:DateTime => Date => "Day",
	) *
	visual(Scatter, color=:red, markersize=1) +
	data(dropmissing(df_, :Ta_measurement)) *
	mapping(
	    :DateTime => Time => "Hour (UTC)",
	    :Tl_mean => "Leaf temperature (°C)",
	    color=:plant => string => "Plant",
...
# ╔═╡ 7808866b-b907-4d12-a32c-d22c5f306919
df_tleaf_H2O = let
	df_ = CSV.read("../05-transpiration/weight_sequence_no_delay.csv", DataFrame)
	df_H2O_minute = transform(
		df_,
		:DateTime => (x -> round.(x, Dates.Minute(1))) => :DateTime,
	)
	df_leaf_temp_minute =  transform(
	...
	leftjoin(df_H2O_minute, df_leaf_temp_minute, on=:DateTime)
end;

# ╔═╡ d7e26b16-2531-40e2-be5f-34117926caaa
md"""
### Plant weight
"""

# ╔═╡ 3a3b8bbd-32da-4ae7-8f3b-89cec6ed41af
p_H2O_2 = let
	df_ = dropmissing(df_tleaf_H2O, [:Tl_mean, :weight])
	# DataFrames.subset!(df_, :weight => (x -> -5 .< x .< 20))
	data(df_) *
	mapping(
	    :DateTime => (x -> Time(x) + Second(0)) => "Hour (UTC)",
	    :weight => (x -> x ./ 100) => "Plant weight (g)",
		layout=:DateTime => Date => "Day",
	) *
	visual(Scatter, color=:red, markersize=1) +
	AlgebraOfGraphics.data(dropmissing(df_tleaf_H2O, :Ta_measurement)) *
	mapping(
	    :DateTime => Time => "Hour (UTC)",
	    :Tl_mean => "Leaf temperature (°C)",
	    color=:plant => string => "Plant",
...
# ╔═╡ f8faae98-141f-4b30-a130-d38f6ca7d855
md"""
Comparing air and leaves temperature intra-daily:
"""

# ╔═╡ 52570490-82c7-438d-b848-7e48d7a1cce6
@bind forced_delay PlutoUI.Slider(0:-1:-10000, show_value=true)

# ╔═╡ 29769798-c6c0-40a8-a7bf-45aaae334b12
climate = CSV.read("../01-climate/climate_mic3.csv", DataFrame);

# ╔═╡ 9c299a08-db51-4c28-aaec-b71cbb9f06e7
climate_days = unique(Date.(climate.DateTime));

# ╔═╡ acf585f1-adc6-407e-a3d7-e57005a19bfa
@bind climate_day_int PlutoUI.Slider(1:length(climate_days))

# ╔═╡ 5049e2d1-131d-42ac-a07e-e46c7246177c
let
	climate_day = climate_days[climate_day_int]
...
manifest_format = "2.0"
project_hash = "30f369509d5dc2b5891885a68a89b46454124160"
