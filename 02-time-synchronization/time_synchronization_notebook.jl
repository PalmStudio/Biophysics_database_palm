...
# ╔═╡ 530001d6-b116-11ed-33c0-21663bc744e5
begin
...
The equipment from Ecotron are all in UTC and synchronized, this include the CO2 fluxes and the data about the door opening of the chamber.
...
Similarly, the data from the precision scale can be checked and eventually corrected using the rapid change in weight when adding or removing a plant from it, because we opened the door of the chamber to do so, so we can get the true timestamp from the database of opening/closing the door. We can also check with the images from the thermal camera.

The precision scale was used to measure the plant transpiration from the weight of the potted plants. The total weight (pot and plant) was continuously measured during the experiment with the connected precision scale. The pot and soil were isolated using a plastic bag, so any change in the pot weight was due to the plant transpiration (+/- the saturation inside the bag).
...
# ╔═╡ 89ff4956-14e7-45e6-93b0-c40941e5704c
md"""
### Importing the data

The method presented above was applyied manually, and the resulting data was logged into this CSV file:
"""
...
# ╔═╡ e243c777-e730-4cb3-aff9-0cd325aca5a8
md"""
## Thermal camera
"""

# ╔═╡ d750e296-856d-433d-908e-37005a2b9c6d
md"""
Build a dataframe with only the data for the thermal camera:
"""

# ╔═╡ 48c9a4d7-a7b4-4fd4-8d33-b2e467f72b5b
df_thermal = select(
    dropmissing(df, [:door_opening, :thermal_camera]),
    :door_opening,
    :thermal_camera,
    [:door_opening, :thermal_camera] => ByRow((x, y) -> ismissing(x) || ismissing(y) ? missing : canonicalize(x - y)) => :error,
    [:door_opening, :thermal_camera] => ((x, y) -> x - y) => :error_ms,
)

# ╔═╡ fd7044ce-2386-48a1-a2f4-3bc4ed7bb828
md"""
### Plotting as is

Matching the door opening time and thermal camera time as is shows no error at the time-scale of the day.
"""

# ╔═╡ cbbb815a-e199-4452-89ed-88c97d756d6b
data(df_thermal) * mapping(:door_opening => "Door opening (UTC)", :door_opening => "Thermal camera time") * visual(Lines, color=:grey, linestyle=:dash) +
data(df_thermal) * mapping(:door_opening => "Door opening (UTC)", :thermal_camera => "Thermal camera time") |>
draw

# ╔═╡ b6ecf96e-099c-4634-bf6c-530dc0a528ff
md"""
*Figure 1. Timestamp of the thermal camera images (y) compared to the time registered with the closest observed door opening (UTC).*
"""

# ╔═╡ c8b6c80f-8d03-4150-8fd4-7adb8b326cce
md"""
### By hour

But looking at the data on an hourly basis shows a systematic delay in the time registered with the camera.
"""

# ╔═╡ 9af55ce0-12c7-4716-8242-d958193a6205
data(df_thermal) *
mapping(
    :door_opening => (x -> Time(x)) => "Door opening (UTC)",
    :door_opening => (x -> Time(x)) => "Thermal camera",
) * visual(Lines, color=:grey, linestyle=:dash) +
data(df_thermal) *
mapping(
    :door_opening => (x -> Time(x)) => "Door opening (UTC)",
    :thermal_camera => (x -> Time(x)) => "Thermal camera"
) * visual(Scatter) |>
draw

# ╔═╡ e8c7cdbc-e230-4d0b-9a87-77c4fec89c8b
md"""
*Figure 2. Hour of the thermal camera images (y) compared to the hour registered with the closest observed door opening (UTC).*
"""

# ╔═╡ 0021a70f-43a9-4cb1-95a2-14c563721272
md"""
### Correction

A correction in the timestamp is then needed. The mean error of the thermal camera timestamps are:
"""

# ╔═╡ f27f3efb-5be0-431b-b607-083128a4adcb
delay_camera = Second(round(mean([i.value for i in skipmissing(df_thermal.error_ms)]) * 1e-3, digits=0))

# ╔═╡ 0c61fa2f-3b4d-48bd-8905-b5c806cf37b8
md"""
We can now add the corrected data to the dataframe:
"""

# ╔═╡ 1f74f124-44eb-4f86-a62e-446813e893f1
df_thermal_cor = transform(df_thermal, :thermal_camera => (x -> x .+ delay_camera) => :thermal_camera_cor)

# ╔═╡ f714a8b1-95ed-4911-859b-d95536a92748
delay_camera

# ╔═╡ 73bdee9d-960a-4fd6-9658-ac4b2f9afc50
md"""
And plot the corrected timestamps in comparison with the previous ones
"""

# ╔═╡ d7552f26-bbc7-4531-81c1-8517de55b07c
data(df_thermal_cor) *
mapping(
    :door_opening => (x -> Time(x)) => "Door opening (UTC)",
    :door_opening => (x -> Time(x)) => "Thermal camera",
...
mapping(
    :door_opening => (x -> Time(x)) => "Door opening (UTC)",
    [:thermal_camera, :thermal_camera_cor] .=> (x -> Time(x)) .=> "Thermal camera",
    color=dims(1) => renamer(["Original", "Corrected"]) => "Camera"
) * visual(Scatter) |>
draw

# ╔═╡ 6234c9c8-ada0-4094-a451-566832cf820a
md"""
*Figure 3. Comparison between the original hour of the thermal camera images (y) and the corrected one, with the closest observed door opening (UTC).*
"""

# ╔═╡ 64fdf10a-0f93-4994-b008-ddc2cd7644c1
md"""
The proposed correction seems adequate.
"""

# ╔═╡ 911ead8b-9351-4428-bd14-52e8754edd32
md"""
### Second methodology, by matching the door opening sensor with seeing the door opening on the thermal camera.
"""

# ╔═╡ 8193a3c1-96c2-4574-9383-95245d00286b
md"""
Import the automatic door opening detection file:
"""
...
md"""
This file is automatically generated from a sensor in the door. It is the absolute reference for door opening monitoring.

Import the second file, which is a file that tags some door opening / closing events from the thermal camera:
"""
...
md"""
Then we transform the opening / closing events in 0-1:
"""
...
md"""
Now we can compare the opening / closing events from both files, day by day:
"""
...
# ╔═╡ d22fcd85-e67d-4b01-96a7-0fd629888ce1
@bind day_int PlutoUI.Slider(1:n_days)

# ╔═╡ 2dc60e28-bf33-4c98-b772-e494fa4f32ca
md"""

Manually optimizing for a delay gives the following delays for each day:
- 2021-03-18: ~3504
- 2021-03-15: 3493
- 2021-03-22: 3483
- 2021-03-26: 3534
- 2021-03-29: 3483 / 3504
- 2021-03-30: 3504
- 2021-04-30: 3483
- 2021-05-03: 3493

!!! note 
	This is only intended as a rough consistency check against the value computed with the first method. The second method is not precise and should not be used as a reference; it simply provides an additional way to verify that the delays considered fall within the same range. In other words, it is just an extra sanity check.
"""

# ╔═╡ 91660fb3-750e-4156-9e40-6328da311591
days = unique(Date.(df_visual_door_opening.DateTime));

# ╔═╡ 05cea038-8c8f-411e-8b72-8ff385ab8226
day = days[day_int]
...
# ╔═╡ 296537e5-3071-40d4-adf7-2a01fadf142a
df_scale = select(
    dropmissing(df, [:scale, :door_opening]),
    :phase, :scale, :door_opening, :thermal_camera,
    [:scale, :thermal_camera] => ByRow((x, y) -> ismissing(y) ? missing : x - y) => :error_camera,
    [:scale, :door_opening] => ((x, y) -> x - y) => :error_door,
)

# ╔═╡ 3f0be081-4d4d-43f5-bf2f-21f68fcb0334
md"""
### Plotting the error as is

The figure below shows that measurements in the phase 4 had a lag that was on the time-scale of a day.
"""

# ╔═╡ 6a699f7c-2825-47c2-b651-54b5dfa2b750
data(df_scale) * mapping(:door_opening => "Door opening (UTC)", :door_opening => "Scale time") * visual(Lines, color=:grey, linestyle=:dash) +
data(df_scale) * mapping(:door_opening => "Door opening (UTC)", :scale => "Scale time", color=:phase) |>
draw

# ╔═╡ 9a7b9a16-b00f-45de-b25d-baee63332fee
md"""
*Figure 4. Timestamp of a radical change in the scale data logging (y, e.g. from 2300 to 0.1) compared to the time registered with the closest observed door opening (UTC).*
"""

# ╔═╡ 78a2652d-8982-4e42-9716-adef0719fe3a
md"""
Looking at the difference between the logged hours, we can see that all phases are close to UTC, except for phase 4 that also have a lag at this time-scale.
"""

# ╔═╡ 42ff75db-1ab1-4206-87cd-85d090c1aea4
data(df_scale) *
mapping(
    :door_opening => (x -> Time(x)) => "Door opening (UTC)",
    :door_opening => (x -> Time(x)) => "Scale time",
) * visual(Lines, color=:grey, linestyle=:dash) +
data(df_scale) *
mapping(
    :door_opening => (x -> Time(x)) => "Door opening (UTC)",
    #[:scale, :scale_cor] .=> (x -> Time(x)) .=> "Scale time",
    #color=dims(1) => renamer(["Original", "Corrected"]) => "Camera"
    :scale => (x -> Time(x)) => "Scale time",
    color=:phase
) * visual(Scatter) |>
draw

# ╔═╡ 9f71cc6d-3faa-4a55-b709-83bae209ecc8
md"""
*Figure 5. Hour of a radical change in the scale data logging (y, e.g. from 2300 to 0.1) compared to the hour registered with the closest observed door opening (UTC).*
"""

# ╔═╡ b5a03d53-a85a-41c8-a27d-4c5a0c701107
md"""
#### Correction
"""
...
delay_scale =
    let
        df_ = combine(
            groupby(df_scale, :phase),
            :error_door => (x -> Second(round(mean([abs(i.value) for i in x]) * 1e-3, digits=0))) => :error_scale,
        )
        transform!(df_, :error_scale => (x -> canonicalize.(x)) => :error_scale_canon)
        df_
    end

# ╔═╡ f37697b0-e95c-4793-bf93-8bbf135de60f
df_scale_cor =
    transform!(
        leftjoin(df_scale, delay_scale, on=:phase),
        [:scale, :error_scale] => ((x, y) -> x .+ y) => :scale_cor
    )

# ╔═╡ 974fcef1-135a-4f3e-b562-b317bf01f64e
data(df_scale_cor) *
mapping(
    :door_opening => "Door opening (UTC)",
    :door_opening => "Scale time",
) * visual(Lines, color=:grey, linestyle=:dash) +
data(df_scale_cor) *
mapping(
    :door_opening => "Door opening (UTC)",
    [:scale, :scale_cor] .=> "Scale time",
    color=dims(1) => renamer(["Original", "Corrected"]) => "Camera"
) * visual(Scatter) |>
draw

# ╔═╡ 799ebbed-7368-48eb-b8cd-a3a0f34f33dc
md"""
*Figure 6. Original timestamp of a radical change in the scale data logging (y, e.g. from 2300 to 0.1) compared to its correction, with a the time registered with the closest observed door opening (UTC).*
"""

# ╔═╡ b616d66b-a9ad-4ffa-83e1-b4d89161c975
data(df_scale_cor) *
mapping(
    :door_opening => (x -> Time(x)) => "Door opening (UTC)",
    :door_opening => (x -> Time(x)) => "Scale time",
...
# ╔═╡ 9c575be6-d698-4a12-9396-48c0b5bd2ce0
md"""
*Figure 7. Orignial hour of a radical change in the scale data logging (y, e.g. from 2300 to 0.1) compared to its correction, with the hour registered with the closest observed door opening (UTC).*
"""

# ╔═╡ bd8c2e20-42f1-4ab1-8d65-6ce0bf892864
md"""
## Saving
"""
...
md"""
We can now write the output computation to disk to be able to use it elsewhere:
"""
...
manifest_format = "2.0"
project_hash = "dc9d989eb026be068e63ec704b2afd6f2258d994"
...
git-tree-sha1 = "0b636a41e46bdf077773713c17154e1ea21678fc"
